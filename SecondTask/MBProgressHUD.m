...
- (void)setTransformForCurrentOrientation:(BOOL)animated withView:(UIView*)view;
- (void)cleanUp;
...
- (id)initWithView:(UIView *)view {
	...
	if ([view isKindOfClass:[UIWindow class]]) {
        [self setTransformForCurrentOrientation:NO withView:view];
	}
	return me;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    NSLog(@"_shouldDisplayInfoIcon%c",_shouldDisplayInfoIcon);

    if (_shouldDisplayInfoIcon) {
        infoButton.hidden=NO;
        infoButton.frame = CGRectMake(CGRectGetMaxX(boxRect)-25 , CGRectGetMaxY(boxRect)-95, 20, 20);
...
- (void)deviceOrientationDidChange:(NSNotification *)notification { 
	...
	if (!superview) {
		...
		[self setTransformForCurrentOrientation:YES withView:nil];
	} else {
...
- (void)setTransformForCurrentOrientation:(BOOL)animated withView:(UIView*)view{
    ...
    if ([[self superview] isKindOfClass:[UIWindow class]] || [view isKindOfClass:[UIWindow class]]) {
